...
getBestSuitableRegressor <- function(dataset, dependent_var, ignore_vars){
  ...
  factors <- factors[!factors %in% ignore_vars]
  ...
  while (length(factors) > 2) {
    ...
    print(paste('Remaining Cols : ', length(factors)))
    ...
    ###
    classifier  <- glm(formula = lm_formula, family = binomial, data = dataset)
    ...
    print(paste('itemToRemove : ' , itemToRemove))
